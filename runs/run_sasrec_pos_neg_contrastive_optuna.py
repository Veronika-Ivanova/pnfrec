...
from modules import SeqRec, SeqRecPosNegContrastive
from models import SASRec, SASRecPosNegContrastive
...
def suggest_hyperparameters(trial):
    ...
    neg_CE_coef = trial.suggest_float("neg_CE_coef", 0.05, 0.5, step=0.05)
    contrastive_coef = trial.suggest_float("contrastive_coef", 0.05, 1, step=0.05)
    print(f"Suggested hyperparameters: \n{pformat(trial.params)}")
    return lr, dropout, hidden_units, num_blocks, neg_CE_coef, contrastive_coef
...
def objective(trial):
    ...
    with mlflow.start_run(run_name="ml1m_sasrec_pos_neg_contrastive"):
        # Get hyperparameter suggestions created by optuna and log them as params using mlflow
        lr, dropout, hidden_units, num_blocks, neg_CE_coef, contrastive_coef = suggest_hyperparameters(trial)
        ...
        seqrec_module = SeqRecPosNegContrastive(model, lr=lr, predict_top_k=10, filter_seen=True, 
                                                neg_CE_coef=neg_CE_coef, contrastive_coef=contrastive_coef)
