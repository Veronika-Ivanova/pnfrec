...
from modules import SeqRec, SeqRecPosContrastive
from models import SASRec, SASRecPosContrastive
...
def suggest_hyperparameters(trial):
    ...
    return lr, dropout, hidden_units, num_blocks, contrastive_coef
...
def objective(trial):
    ...
    with mlflow.start_run(run_name="ml1m_sasrec_pos_contrastive"):
        # Get hyperparameter suggestions created by optuna and log them as params using mlflow
        lr, dropout, hidden_units, num_blocks, contrastive_coef = suggest_hyperparameters(trial)
        mlflow.log_params(trial.params)
        mlflow.log_params({"maxlen": MAX_LENGTH, "batch_size": BATCH_SIZE})

        model = SASRecPosContrastive(
            item_num=item_count,
            add_head=add_head,
            maxlen=MAX_LENGTH,
            num_heads=1,
        ...
        seqrec_module = SeqRecPosContrastive(model, lr=lr, predict_top_k=10, filter_seen=True, 
