...
from modules import SeqRec, SeqRecPosNeg
from models import SASRec, SASRecPosNeg
...
def suggest_hyperparameters(trial):
    ...
    return lr, dropout, hidden_units, num_blocks, neg_CE_coef
...
def objective(trial):
    ...
    with mlflow.start_run(run_name="ml1m_sasrec_pos_neg"):
        # Get hyperparameter suggestions created by optuna and log them as params using mlflow
        lr, dropout, hidden_units, num_blocks, neg_CE_coef = suggest_hyperparameters(trial)
        ...
        seqrec_module = SeqRecPosNeg(model, lr=lr, predict_top_k=10, filter_seen=True, neg_CE_coef=neg_CE_coef)
